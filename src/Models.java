    ...
    public void model() throws SQLException {
        try {
            ...
            statement.execute("create table if not exists " + TABLE_GPM +
                    ...
                    "bdo_id integer not null,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,foreign key (bdo_id) references bdo(id))");

            statement.execute("create table if not exists " + TABLE_MEMBER +
                    "(id integer primary key autoincrement,name text not null," +
                    "email text not null,password text not null,area text not null,pincode integer not null," +
                    "status text default 'not assigned',age integer not null,no_of_days_worked int,wage_amount float,wage_approved boolean default 'false'," +
                    "gpm_id integer not null,created_at DATETIME DEFAULT CURRENT_TIMESTAMP,foreign key (gpm_id) references gpm(id))");
            ...
            statement.execute("create table if not exists " + TABLE_PROJECT +
                    ...
                    "start_date date not null,end_date date not null,is_alloted boolean default 'false',created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
            ...
            statement.execute("create table if not exists " + TABLE_PROJECT_MEMBER +
                    ...
                    "gpm_id integer not null,status boolean default 'false',created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
            ...
            statement.execute("create table if not exists " + TABLE_COMPLAINTS +
                    ...
                    "status_bdo boolean default 'false',created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
